...
void ASWeapon::Fire()
{
	...
	if (MyOwner)
	{
		...
		if (GetWorld()->LineTraceSingleByChannel(Hit, EyeLocation, TraceEnd, COLLISION_WEAPON, QueryParms))
		{
			...
			UGameplayStatics::ApplyPointDamage(HitActor, ActualDamge, ShotDirection, Hit, MyOwner->GetInstigatorController(), MyOwner, DamageType);
...
void ASWeapon::PlayFireEffects(FVector TraceEnd)
{
	if (MuzzleEffect)
	{
		UGameplayStatics::SpawnEmitterAttached(MuzzleEffect, MeshComp, MuzzleSocektName);
	}

	if (TraceEffect)
	{
		FVector MuzzleLocation = MeshComp->GetSocketLocation(MuzzleSocektName);
